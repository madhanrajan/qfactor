...
def process_data(jsondata):
    ...
    filling_fraction = (
        math.pi*float((params["radius"]))**2)/(float(params["distance"])**2)

    (n, k, wl) = interpolate(Element.objects.get(
        name=params["nanorod_material"]), wl_init=params["initial_wavelength"], wl_fin=params["final_wavelength"])

    E = get_permitivity(n, k, radius, distance=distance, d_perm_real=float(
        params["dielectric_material_real"]), d_perm_im=float(params["dielectric_material_complex"]))
...
def get_permitivity(x, y, radius, distance, d_perm_real=1, d_perm_im=0):
    ...
    for i in range(len(x)):
        ...
        Exy = d_perm*(oneplusp*k + oneminusp*d_perm) / \
            (oneplusp*d_perm + oneminusp*k)
...
def interpolate(element, wl_init=400, wl_fin=900):
    ...
    # list of wavelength i.e column 1
    wavelength = list(map(lambda x: x[0], data))
