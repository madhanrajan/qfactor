...
def get_reflection(M, theta, wl, eps_super, eps_sub):
    ...
    r = ((m11 + m12*q_super(eps_super, wl, theta))*q_sub(eps_sub, wl, theta) - (m21+m22*q_super(eps_super, wl, theta))) / \
        ((m11+m12*q_super(eps_super, wl, theta))*q_sub(eps_sub,
                                                       wl, theta) + (m21*m22*q_super(eps_super, wl, theta)))
    return r
...
def get_transmission(M, theta, wl, eps_super, eps_sub):
    ...
    t = 2*q_sub(eps_sub, wl, theta)/((m11 + m12 * q_super(eps_super, wl, theta))
                                     * q_sub(eps_sub, wl, theta) + (m21 + m22*q_super(eps_super, wl, theta)))
